...
static int tbsqbox_init(winlirc_api const* winlirc) {
					...
					if (hr == S_OK)
					{
						...
						while(pEnumCat->Next(1, &pMoniker, &cFetched) == S_OK)
						{
							...
							{							
								...
								CComPtr <IKsPropertySet> pKsTunerPropSet = nullptr;
								// query for interface
								hr = pFilter->QueryInterface(IID_IKsPropertySet, (void **)&pKsTunerPropSet);
								if (pKsTunerPropSet)
								{
									DWORD type_support;
									hr = pKsTunerPropSet->QuerySupported(KSPROPERTYSET_QBOXControl,
										KSPROPERTY_CTRL_IR,
										&type_support);
									if ( SUCCEEDED(hr) && (type_support & KSPROPERTY_SUPPORT_GET) )
									{
										PCHAR t=strstr(szFriendlyName,"Tuner");
										if (t) *t=0;
...
static void tbsqbox_loadSetupGui() {
...
static int tbsqbox_sendIR(struct ir_remote *remote, struct ir_ncode *code, int repeats) {

	return 0;
}

static int tbsqbox_decodeIR(struct ir_remote *remotes, char *out, size_t out_size) {
...
static hardware const* tbsqbox_getHardware() {
	return &tbsqbox_hw;
}
...
WL_API plugin_interface const* getPluginInterface() {
	static constexpr plugin_interface p{
		...
		.init = tbsqbox_init,
		.deinit = tbsqbox_deinit,
		.hasGui = tbsqbox_hasGui,
		.loadSetupGui = tbsqbox_loadSetupGui,
		.sendIR = tbsqbox_sendIR,
		.decodeIR = tbsqbox_decodeIR,
		.setTransmitters = nullptr,
		.getHardware = tbsqbox_getHardware,
		.hardware = &tbsqbox_hw,
	};
	return &p;
}