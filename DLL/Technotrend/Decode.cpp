...
#pragma warning (disable: 4018)	// disable signed/unsigned mismatch warnings
...
lirc_t get_next_rec_buffer(lirc_t maxusec)
{
	...
	else
	{
		if(rec_buffer.wptr<RBUF_SIZE)
		{
			...
			receive->waitTillDataIsReady(2*maxusec<100000 ? 100000:2*maxusec);
		
			if(!receive->getData(&data))
			{
...
bool decodeCommand(struct ir_remote *remotes, char *out)
{
	...
	if(!clear_rec_buffer()) return false;
	...
	while(remote)
	{
		...
		if(receive_decode(remote,&pre,&code,&post,&repeat_flag,
