...
static bool read_config_recursive(FILE* f, winlirc::istring_view name, int depth, std::vector<ir_remote>& res);
...
static std::vector<ir_remote>& sort_by_bit_count(std::vector<ir_remote>& remotes)
{
	std::sort(remotes.begin(), remotes.end(), [](auto& a, auto& b) { return bit_count(&a) < bit_count(&b); });
	return remotes;
}
...
{
	std::vector<ir_remote> res;
	read_config_recursive(f, name, 0, res);
	sort_by_bit_count(res);
	return res;
}
...
static bool read_config_recursive(FILE* f, winlirc::istring_view name, int depth, std::vector<ir_remote>& res)
{
	...
	int argc;
	ir_remote* currentRemote = nullptr;
	...
	while (fgets(bufx, LINE_LEN, f) != nullptr)
	{
		...
		if (!val.empty()) {
			...
			if ("include" == key) {
				...
				{
					int const save_line = line;
					read_config_recursive(childFile, fullPath, depth + 1, res);
					fclose(childFile);
					line = save_line;
				}
			}
				...
				if ("codes" == val) {
					...
					set_protocol(currentRemote, RAW_CODES);
					raw_code.code = 0;
					raw_codes.clear();
					...
					while (!parse_error && !val2.empty())
					{
						...
						auto const node = defineNode(code, val2);
						val2 = strtok(buf, whitespace);
					}
				...
				if ("codes" == val) {
					...
					if (!checkMode(mode, ID_codes,
						"end codes")) break;
					currentRemote->codes = std::move(codes_list);
					codes_list.clear();
					...
					if (!checkMode(mode, ID_raw_codes,
						"end raw_codes")) break;
					currentRemote->codes = std::move(raw_codes);
					raw_codes.clear();
					mode = ID_remote;     /* switch back */
				}
				else if ("remote" == val) {
					...
					currentRemote->last_code = nullptr;
				...
				switch (mode) {
				case ID_remote:
					argc = defineRemote(key, val, val2, currentRemote);
					break;
				case ID_codes:
					code = defineCode(key, val, &name_code);
					while (!parse_error && !val2.empty())
